...
function decode_wmr100_forecast(?string $rawHex): ?int
{
    ...
    // Pressure packet: status, 0x46, pressure low, forecast/high-pressure byte,
    // relative-pressure low, previous/unknown nibble byte, checksum lo/hi.
    if(count($bytes)!==8 || ($bytes[1]??null)!==0x46) return null;
    return (($bytes[3] >> 4) & 0x0F);
}

function store_wmr100_forecast_from_pressure(array &$a, array $data, ?string $timestamp, ?float $epoch): void
{
    $code = decode_wmr100_forecast((string)($data['raw_hex']??''));
    if($code===null) return;
...
function weather_value(string $key, mixed $value, string $family='wmr100'): string
{
    ...
    if(in_array($key,['pressure','barometer','altimeter'],true))return number_format($n,1,',','.').' hPa';
...
function store_weather_record(array &$a, array $record, string $packetName, ?string $timestamp, ?float $epoch): void
{
    ...
    foreach($record as $field=>$value){
        ...
        $a['weather_latest'][$common]=['value'=>$value,'timestamp'=>$timestamp,'epoch'=>$epoch,'packet'=>$packetName,'usUnits'=>$record['usUnits']??null,'native_field'=>$field];
        if(is_numeric($value)){
...
function process_wmr100_record(array &$a,string $event,array $data,?string $timestamp,?float $epoch): void
{
    switch($event){
        ...
        case 'packet_valid':
            ...
            if($packet==='pressure') store_wmr100_forecast_from_pressure($a,$data,$timestamp,$epoch);
...
function weather_groups(): array
{
    return [
      ...
      'pressure'=>['title'=>'Pressione','fields'=>['pressure'=>'Pressione stazione','altimeter'=>'Altimetro console','forecastIcon'=>'Previsione console']],
    ...
    <?php foreach($group['fields'] as $field=>$flabel):$r=reading($a,$field);?>
      <?php if($field==='forecastIcon'):?>
        ...
        <div class="forecast-metric">
          ...
          <div class="forecast-copy"><span class="metric-name"><?=h($flabel)?></span><strong><?=h($fi['label'])?></strong><span class="forecast-code">Codice console <?=h((string)$fc)?></span><span class="metric-age">letto <?=h(format_age(reading_age($r)))?> · <?=h(local_timestamp($r['timestamp'],false))?></span></div>
        </div>
        <?php else:?>
